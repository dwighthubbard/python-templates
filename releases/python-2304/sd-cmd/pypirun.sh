...
function bootstrap_pypirun {
    ...
    ORIG_PYTHON_BOOTSTRAP_SKIP_SCREWDRIVERCD="$PYTHON_BOOTSTRAP_SKIP_SCREWDRIVERCD"
    PYTHON_BOOTSTRAP_SKIP_SCREWDRIVERCD=True
    sd-cmd exec python-2304/python_bootstrap@latest >> $PYPIRUN_LOGFILE 2>&1
    source /tmp/python_bootstrap.env||/bin/true
    PYTHON_BOOTSTRAP_SKIP_SCREWDRIVERCD="$ORIG_PYTHON_BOOTSTRAP_SKIP_SCREWDRIVERCD"
    if [ "$PYTHON_BOOTSTRAP_SKIP_SCREWDRIVERCD" = "" ]; then
        unset PYTHON_BOOTSTRAP_SKIP_SCREWDRIVERCD
    fi
}
